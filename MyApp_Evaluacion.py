...
#     st.dataframe(df_form)
    ...
    if calcular:
        proba = model.predict_proba(df_form)[0] #[.49,.51]
        resultado = model.predict(df_form)
        
        if resultado == 0:
            st.markdown(f'La persona **no ganará** más de $50K con probabilidad de {round(proba[0]*100,2)}% ') # .29
        else:
            st.markdown(f'La persona **ganará** más de $50K con probabilidad de {round(proba[1]*100,2)}% ')  # .71
